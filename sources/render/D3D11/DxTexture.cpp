...
namespace
{
	...
	{
		UINT bindFlags = 0;
		if (format & ITexture::BIND_DEPTH_STENCIL)	 bindFlags |= D3D11_BIND_DEPTH_STENCIL;
		if (format & ITexture::BIND_RENDER_TARGET)	 bindFlags |= D3D11_BIND_RENDER_TARGET;
		if (format & ITexture::BIND_SHADER_RESOURCE) bindFlags |= D3D11_BIND_SHADER_RESOURCE;
		return bindFlags;
	}
...
namespace render
{
	...
	//------------------------------------------
	DXTexture::DXTexture(const Desc& desc) : ITexture(desc), m_tex(nullptr)
	{
	...
	bool DXTexture::init(const ITexture::Data* data, uint size)
	{
		...
		if (bindFlags & D3D11_BIND_RENDER_TARGET)	success &= _createRTView();
		if (bindFlags & D3D11_BIND_SHADER_RESOURCE) success &= _createSRView();
		if (bindFlags & D3D11_BIND_DEPTH_STENCIL)	success &= _createDSView();

		return success;
	}

	//------------------------------------------
	bool DXTexture::init(const ComPtr<ID3D11Texture2D>& tex2D)
	{
		...
		HRESULT hr = m_tex2D->QueryInterface(IID_ID3D11Resource, (void **)&m_tex);
	...
	{
		if (m_SRView)
			dxDevice().immediateContext()->GenerateMips(m_SRView.get());
	}
	...
	bool DXTexture::_createTex1D(UINT bindFlags, const Data* data, uint size)
	{
		...
		dxDesc.Usage			= D3D11_USAGE_DEFAULT; // default usage?
		dxDesc.MiscFlags		= (m_desc.mipLevels == 0) ? D3D11_RESOURCE_MISC_GENERATE_MIPS : 0;
	
		std::vector<D3D11_SUBRESOURCE_DATA> dxData;
		if (size > 0)
		{
			dxData.resize(size);
		...
		// set the base texture we'll use in the render system.
		hr = m_tex1D->QueryInterface(IID_ID3D11Resource, (void **)&m_tex);
		if (FAILED(hr) || dxDevice().hasError())
		{
			ERROR_MSG("Failed to create 1D texture.\n Desc: " + dxDevice().getErrorDesc());
			return false;
	...
	bool DXTexture::_createTex2D(UINT bindFlags, const Data* data, uint size, bool cubeMap)
	{
		...
		dxDesc.Usage				= D3D11_USAGE_DEFAULT; // default usage?
		dxDesc.MiscFlags			= (m_desc.mipLevels == 0) ? D3D11_RESOURCE_MISC_GENERATE_MIPS : 0;
		dxDesc.SampleDesc.Count		= m_desc.sample.count;
		dxDesc.SampleDesc.Quality	= m_desc.sample.quality;
		...
		// set the base texture we'll use in the render system
		hr = m_tex2D->QueryInterface(IID_ID3D11Resource, (void **)&m_tex);
		if (FAILED(hr) || dxDevice().hasError())
		{
			ERROR_MSG("Failed to create 2D texture.\nDesc: " + dxDevice().getErrorDesc());
			return false;
	...
	bool DXTexture::_createTex3D(UINT bindFlags, const Data* data, uint size)
	{
		...
		// set the base texture we'll use in the render system
		hr = m_tex3D->QueryInterface(IID_ID3D11Resource, (void **)&m_tex);
		if (FAILED(hr) || dxDevice().hasError())
		{
			ERROR_MSG("Failed to create 3D texture.\nDesc: " + dxDevice().getErrorDesc());
			return false;
